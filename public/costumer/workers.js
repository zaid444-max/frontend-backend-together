...
addCustomerButt.addEventListener('click', async () => {
  ...
  const addedWoriRes = await response.json();
  const workers = addedWoriRes.workers;
...
async function fetchCustomers(workers) {
  brandList.length = 0;
  const scrollPosition = tableDiv.scrollTop;
  tableTbody.innerHTML = '';
  workers.sort((a, b) => b.id - a.id)
  workers.forEach((customer, index) => {
...
async function deleteBrand(event) {
  ...
  if (delResp.invoice) {
    Toastify({
      text: `You can't delete this worker, because it's used.`,
      duration: 2000,
      gravity: 'top', // or 'top'
